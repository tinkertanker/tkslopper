...
Persisted attempt records begin only after quota admission, and an `attempt_started` intent is written before the physical provider call. The 24-hour aggregates exclude those unfinished intents. Finalized failures can retain conservative token and cost estimates, so accounted values are not necessarily realized usage. Stale intents and their reservation ceilings are reported separately.
